...
depends_on:
  ...
  - F2S-DOC-STORE-001
  - F2S-DOC-ADR-001
review_score_ref: F2S-SCORE-DOC-EXPORT-001-R3B
...
- 内置生成的Spine Editor 4.2.43 JSON、透明attachment PNG与atlas packing/input manifest；
- 用户本地 Spine Professional CLI 的受控实验调用（取得本机证据前为 unverified）；
- 结构验证、语义 round-trip、失败隔离及兼容清单。

V1 首个正式兼容目标选定为 **仅 Spine Editor 4.2.43**。产品不捆绑 Spine Editor，也不捆绑任何官方 Spine Runtime；内部预览直接渲染 Rig IR。内置Spine输出边界固定为`character.spine.json`、透明attachment PNG和`atlas-input-manifest.json`；`.atlas/.spine/.skel`没有内置writer，只有用户本地、合法安装且本轮证明为4.2.43的`Spine.com`可以在可选验证或转换步骤中生成。CLI缺失或失败时不得回退到自研writer。
...
依照 `F2S-DOC-ADR-001`，所有编辑、自动分析、人工修正、撤销重做和导出均先提交为 Rig IR。本文据此规定 PSD、透明PNG、Spine JSON、atlas-input manifest、预览缓存及 CLI 产物都是可再生派生物，不能反向成为项目真相源。派生并不等于同一writer所有权：`.atlas/.spine/.skel`必须保留CLI生成来源。
...
桌面应用使用 PixiJS 8 渲染自有 Rig IR，不加载官方 Spine Runtime。Spine JSON、透明attachment PNG和atlas-input manifest是内置边界输出，不是应用内部运行格式；`.atlas/.spine/.skel`是用户本地Professional CLI拥有的输出边界。这样既保证内部模型可控，也满足发布依赖许可证白名单。
...
Rust Application Service（唯一工作流事实源）
   ...
   ├── Atlas Input Manifest Adapter（只描述packing输入，不写.atlas）
   ├── Export Orchestrator
...
- 内置源附件始终使用straight alpha并保持透明背景；PMA、padding、旋转、页尺寸等只写入atlas-input packing policy。实际atlas page及`.atlas`描述文件只可由用户本地4.2.43 Professional CLI生成。
...
### F2S-EXP-ATLAS-INPUT-001：packing/input manifest

`atlas-input-manifest.json`是内置、开放且确定性的CLI输入清单，不是Spine `.atlas`文件。它至少固定：

- `schemaVersion`、目标`editorVersion=4.2.43`、ExportProfile hash和源Rig revision；
- 按logicalPath排序唯一的attachment项：透明PNG相对路径、普通SHA-256、宽高、straight-alpha、颜色空间、slot/skin/attachment映射；
- packing策略：最大页尺寸、padding、rotation、stripWhitespace、scale、requestedPma与命名模板；
- `outputOwnership={atlasDescriptor:"localProfessionalCliOnly",spineProject:"localProfessionalCliOnly",skeletonBinary:"localProfessionalCliOnly"}`；
- manifest自身domain/artifact hash以及与`character.spine.json`、`images/`清单的交叉hash。

内置阶段只验证该manifest与透明PNG/JSON逐项闭合，绝不把manifest改扩展名伪装为`.atlas`。如CLI执行成功，其输出进入`cli-generated/<operation-id>/`，并由`SpineCliEvidence`逐文件引用；`.atlas/.spine/.skel`离开该provenance即视为外部未知文件，不能进入验证结论。
...
- `.atlas/.spine/.skel`输出路径必须位于本次operation的`cli-generated`或round-trip staging，创建前断言不存在，完成后以文件系统审计+hash证明由该CLI进程产生；内置Adapter没有这些扩展名的写入能力。
...
`.atlas`packing与`.skel`binary的确切CLI export-settings/参数模板必须由合法4.2.43实测fixture冻结后才能启用；在此之前保持`UNVERIFIED/not-run`。无论模板是否已验证，产品都不得以自研代码补写这些格式。用户手工放入或旧operation遗留的同名文件必须在本轮开始前隔离，不能被收编为本轮CLI输出。

每轮验证的`SpineCliEvidence`至少包含executable canonical path的哈希、mutex lease ID、operation ID、参数模板/hash、probe输出摘要、每个进程起止时间/退出码、输入JSON/透明PNG/atlas-input manifest三者hash、CLI输出逐文件路径/扩展名/hash/created-before-after审计，以及重导JSON hash及其中实际`skeleton.spine`。任何`.atlas/.spine/.skel`必须能反向定位到该evidence；实际patch证明来自本轮转换产物，不从上一次probe、用户设置或旧文件继承。
...
| L2 | 本地Spine 4.2.43 Professional CLI导入JSON→生成`.spine`→重导JSON，并按请求由同一CLI生成`.atlas/.skel` | 无本机证据时标记`not-run`；内置JSON/透明PNG/atlas-input manifest仍可提交，但不得伪装这些CLI格式已生成/验证 |
...
```json
{
  ...
  "exportMode": "release",
  "writerOwnership": {
    "spineJson": "builtInAdapter",
    "transparentPng": "builtInAdapter",
    "atlasInputManifest": "builtInAdapter",
    "atlas": "localProfessionalCli",
    "spineProject": "localProfessionalCli",
    "skeletonBinary": "localProfessionalCli"
  },
  "validation": {
    "l0": "pass",
    "l1": "pass",
...
Candidate在L0/L1后可跳过CLI/L3，但只能提交为`exported_unverified`，其内置snapshot必须包含JSON、透明PNG与atlas-input manifest且不得包含`.atlas/.spine/.skel`。Release不得跳过`optional-cli-roundtrip`（对Release实际为required）和L3；如果release清单包含这些CLI格式，必须来自本轮`cli-generated`证据。状态机中的`committed`表示导出快照原子提交，不等价于`spine_verified`。
...
- 把“内置生成JSON/透明PNG/atlas-input manifest”和“本地Professional CLI生成/验证`.atlas/.spine/.skel`”显示为不同writer、不同步骤和不同provenance区。
- 未配置CLI时仍允许以Candidate profile导出PSD/透明PNG/Rig IR/候选JSON/atlas-input manifest，但显示`exported_unverified`且CLI格式计数为0；Release profile必须禁用并显示缺失证据。
- 所有会导致丢字段的情况必须阻断，不提供“忽略并继续”默认按钮。
- 展示功能支持矩阵和每个 `not-run` 原因。
- 人工审批展示/记录操作者与9位纳秒UTC、target Rig revision/hash、DependencySnapshot/稳定PrioritySlice及transport PriorityRegistryObservation、stable GateOutcome、DecisionPolicy、report artifact、精确active ack、静态GovernedWaiverBinding，并按current Priority/Waiver→ActorRegistry/enrollment/trust+revocation bundle/clockEpoch/effectiveNow重验proposal/attestation与动态validation/ActorRole/clock evidence；本地显示时间只作UI，不替代规范UTC或当前受信时钟证据。
...
每个合法fixture必须冻结内置`character.spine.json`、全部透明attachment PNG和`atlas-input-manifest.json`的bytes/hash，并有“CLI disabled时递归扫描`.atlas/.spine/.skel`数量=0”的断言。六个Spine golden另有CLI-enabled变体：本轮operation开始前清空/隔离同名文件，运行合法4.2.43 Professional CLI后逐项冻结CLI产物hash、生成时间/operation和重导语义；不存在CLI证据时这些预期只能为`not-run`，不能把仓库预置文件当PASS。

### 稳定验收映射

- `F2S-AC-EXP-001`：同一revision与profile连续导出两次，Rig IR、透明PNG/PSD语义、JSON和atlas-input manifest一致；关闭CLI时两次snapshot均无`.atlas/.spine/.skel`；不支持特性必须硬错误且不得静默丢失。
- `F2S-FR-EXP-002`：PSD reopen 逐字段证明 canvas、组/层级、名称、顺序、可见性、opacity/alpha、origin/pivot、像素层与 manifest 一致；任一 P0 字段缺失即失败。
- `F2S-AC-EXP-002`：所有release JSON明确声明`4.2.43`；合法Professional CLI的L2 round-trip对已支持fixture通过，并记录脱敏命令摘要、退出码和逐文件产物hash；任何`.atlas/.spine/.skel`均证明由本轮4.2.43 CLI生成，内置writer写入探针必须为0。
- `F2S-AC-EXP-003`：没有Spine、版本不符或CLI失败时，核心编辑和Rig IR预览继续工作，内置JSON/透明PNG/atlas-input manifest只可标记`exported_unverified`或`incompatible`，且不存在无provenance的`.atlas/.spine/.skel`。
...
- 不以内置writer逆向、生成或改写`.atlas/.spine/.skel`；三者仅接受用户本地合法Professional CLI 4.2.43本轮输出。
