...
review_score_ref: F2S-SCORE-DEVPLAN-M00-001-R1
...
M00 的 bootstrap 证据也必须逐项遵守 M02 冻结的唯一 `EvidenceEnvelope` 字段合同：`schemaVersion/evidenceId/taskId/status/command/exitCode/startedAtUtc/endedAtUtc/runner/toolVersions/inputHashes/outputHashes/logRefs/reportRefs/capabilityState/externalBlockers/previousEvidenceRef/notRunReason/payload`。M02 实现后必须用 `schemas/src/evidence.schema.json` 回验 M00 历史证据；计划/执行顺序不允许产生第二套 envelope。
...
- 上游文档：F2S-DOC-DEVPLAN-INDEX-001；下游 DEV：F2S-DEV-M01-001、F2S-DEV-M01-002、F2S-DEV-M00-005。
...
- evidence.json 必须通过唯一 `EvidenceEnvelope`；本任务探测详情只置于 `payload`；初态 status=NOT_RUN、capabilityState=UNVERIFIED，`notRunReason` 按合同显式为 null 或原因。
...
- evidence.json 必须通过唯一 `EvidenceEnvelope`；inventory、license report 和 package inclusion diff 由 `reportRefs` 引用，任务专属分类只置于 `payload`；初态 NOT_RUN/UNVERIFIED。
...
- evidence.json 必须通过唯一 `EvidenceEnvelope`；`inputHashes` 指向 source/manifest，`outputHashes` 指向两 PNG，fixture 专属字段只置于 `payload`；初态 NOT_RUN/UNVERIFIED。
...
- command: npm run test:integration
- tests: F2S-TST-073、F2S-TST-E2E-PERF-006；向量为 8GB、低于 8GB、无 CUDA、错误 driver、OOM、取消。
...
- evidence.json 必须通过唯一 `EvidenceEnvelope`；OS build/GPU/driver/Python/CUDA 组合只置于 `payload`，runner 保持标准字段；未在授权硬件执行时 NOT_RUN/UNVERIFIED 并填写 `notRunReason/externalBlockers`。
...
- 上游 DEV：F2S-DEV-M00-001、F2S-DEV-M00-002、F2S-DEV-M00-003；下游 DEV：F2S-DEV-M05-006、F2S-DEV-M06-003、F2S-DEV-M08-003、F2S-DEV-M08-004、F2S-DEV-M08-006、F2S-DEV-M08-007。
- 携带项：P0/P1 无；P2 无。
...
- fixtures/m00/spine42-probe/rig-ir.json、skeleton.json、attachments/body.png、attachments/weapon.png、capability-manifest.json：开放格式最小 fixture 与唯一 `F2S-SPINE-CAP-4.2.43-001` 静态能力清单，owner 本任务。
...
- writes: fixtures/m00/spine42-probe/rig-ir.json；fixtures/m00/spine42-probe/skeleton.json；fixtures/m00/spine42-probe/attachments/body.png；fixtures/m00/spine42-probe/attachments/weapon.png；fixtures/m00/spine42-probe/capability-manifest.json
- steps: 1) 建最小 bone/slot/skin；2) 固定 Spine skeleton version 4.2.43；3) 领域与 Rig IR 使用 i64 tick+rational timeBase（默认 1/30000 秒），仅在 Spine42 Adapter 边界把 tick 转为秒；4) 只引用透明 PNG；5) 写唯一 ID=`F2S-SPINE-CAP-4.2.43-001`、静态 feature allowlist、source/fixture hash 与 `editorRoundTripStatus=EXTERNAL|UNVERIFIED|VERIFIED`；6) 规范排序并写 golden hash。
- command: npm run test:spine
...
- evidence.json 必须通过唯一 `EvidenceEnvelope`；toolVersions 精确写 4.2.43，能力清单/往返专属字段只置于 `payload`；无合法工具时 capabilityState=EXTERNAL，并填写 `notRunReason/externalBlockers`。
