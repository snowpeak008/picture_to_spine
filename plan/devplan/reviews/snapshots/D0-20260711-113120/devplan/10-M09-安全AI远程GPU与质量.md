...
2. **追踪**：F2S-FR-RGPU-001、F2S-NFR-PRIV-001、F2S-NFR-SEC-002、F2S-NFR-SEC-005、F2S-R3A-B12-P2-001；测试 F2S-TST-RGPU-005、F2S-TST-E2E-SEC-005、F2S-TST-077；上游 F2S-DEV-M00-004、F2S-DEV-M01-001、F2S-DEV-M02-005、F2S-DEV-M04-004；下游 F2S-DEV-M09-003、F2S-DEV-M09-005、F2S-DEV-M10-001；P0，B12 primary owner。
...
##### F2S-WU-M09-001-01 — AppContainer启动与ACL
   ...
   - command：npm run build:core；npm run typecheck。
   - tests：正常 echo worker、项目根读取、用户目录读取、Credential Manager、任意子进程、未授权网络。
   - evidence：evidence/M09/F2S-DEV-M09-001/launcher-tests.log、sandbox-profile-hash.json。
   - dependsOn：[F2S-DEV-M01-001, F2S-DEV-M02-005, F2S-DEV-M04-004]。
   - parallelSafety：锁 repo:crates/adapters/windows-appcontainer、repo:config/sandbox/windows-appcontainer-v1.json；运行时独占 job sandbox。
...
##### F2S-WU-M09-001-02 — 隔离逃逸与外联探针
   ...
   - evidence：evidence/M09/F2S-DEV-M09-001/security-probes.json、integration.log。
   - dependsOn：[F2S-WU-M09-001-01]。
   - parallelSafety：锁 repo:tests/security/appcontainer/escape.spec.ts；测试机串行运行，避免 ACL/网络观测互扰。
...
##### F2S-WU-M09-001-03 — PythonCUDA与GPU组合矩阵
   ...
   - command：npm run build:ai-pack；npm run test:integration；npm run release:verify。
   - tests：F2S-TST-RGPU-005 与 F2S-R3A-B12-P2-001 exact matrix。
   - evidence：evidence/M09/F2S-DEV-M09-001/evidence.json、python-cuda-gpu-matrix.json；缺硬件/pack 的格子含 blockedClaim。
   - dependsOn：[F2S-DEV-M00-004, F2S-WU-M09-001-02]。
   - parallelSafety：测试机/GPU 独占；锁 repo:tests/security/appcontainer/python-cuda-gpu-matrix.spec.ts。
...
##### F2S-WU-M09-002-01 — 私有端点与传输计划
   ...
   - command：npm run build:core；npm run typecheck；npm test。
   - tests：TLS 错误、重定向、DNS/endpoint 变化、token 缺失、未批准传输、超限响应。
   - evidence：evidence/M09/F2S-DEV-M09-002/provider-contract.log、transfer-plan-fixtures.json。
   - dependsOn：[F2S-DEV-M02-005, F2S-DEV-M04-004, F2S-DEV-M08-008]。
   - parallelSafety：锁 repo:crates/adapters/private-remote-gpu/src/lib.rs；每 endpoint/job 独立 lease。
...
##### F2S-WU-M09-002-02 — 远端作业与删除收据
   ...
   - evidence：evidence/M09/F2S-DEV-M09-002/evidence.json、mock-receipts.json；真实端点缺失时 remote-e2e=NOT_RUN/EXTERNAL。
   - dependsOn：[F2S-WU-M09-002-01]。
   - parallelSafety：锁 repo:crates/adapters/private-remote-gpu/src/receipt.rs、repo:tests/remote-gpu；运行时 jobId 串行。
...
##### F2S-WU-M09-003-01 — 文件边界攻击集
   ...
   - evidence：evidence/M09/F2S-DEV-M09-003/file-attacks.json、integration.log。
   - dependsOn：[F2S-DEV-M02-001, F2S-DEV-M04-001, F2S-DEV-M08-005]。
   - parallelSafety：锁 repo:tests/security/adversarial/file-boundary.spec.ts、repo:fixtures/security/files；独立临时根。
...
##### F2S-WU-M09-003-02 — 进程与IPC攻击集
   ...
   - evidence：evidence/M09/F2S-DEV-M09-003/process-attacks.json。
   - dependsOn：[F2S-DEV-M01-003, F2S-DEV-M08-006, F2S-DEV-M09-001]。
   - parallelSafety：锁 repo:tests/security/adversarial/process-boundary.spec.ts；AppContainer 测试串行。
...
##### F2S-WU-M09-003-03 — 远端凭据与隐私攻击集
   ...
   - evidence：evidence/M09/F2S-DEV-M09-003/evidence.json、attack-catalog-results.json。
   - dependsOn：[F2S-DEV-M09-002, F2S-WU-M09-003-01, F2S-WU-M09-003-02]。
   - parallelSafety：锁 repo:tests/security/adversarial/remote-boundary.spec.ts、repo:tools/security；只连 mock endpoint。
...
##### F2S-WU-M09-004-01 — 许可策略与组件清单
   ...
   - command：npm run build:core；npm run build:ai-pack。
   - tests：MIT/Apache/BSD 等 allowlist fixture、unknown、双许可、缺 source/hash、模型仅有代码许可。
   - evidence：evidence/M09/F2S-DEV-M09-004/inventory-fixtures.json、build-inventories.log。
   - dependsOn：[F2S-DEV-M00-002, F2S-DEV-M01-002, F2S-DEV-M09-001]。
   - parallelSafety：锁 repo:policy/licensing、repo:tools/compliance/inventory.ps1；读取 lockfiles 不写。
...
##### F2S-WU-M09-004-02 — 发布许可硬门
   ...
   - evidence：evidence/M09/F2S-DEV-M09-004/evidence.json、sbom-validation.json、forbidden-component-scan.json；法务意见缺失标 EXTERNAL。
   - dependsOn：[F2S-WU-M09-004-01]。
   - parallelSafety：锁 repo:tools/compliance/verify-policy.ps1、repo:tests/compliance；构建 staging 只读。
...
3. **输入**：F2S-ENV-WIN-001、F2S-FIX-PROJ-001、renderer telemetry、DPI matrix、8GB GPU（若存在）、cold/warm run protocol。
...
##### F2S-WU-M09-005-01 — 核心性能预算
   ...
   - reads：Desktop instrumentation、标准 fixture、NFR thresholds。
   - writes：tools/benchmark/run-performance.ps1、tests/performance/performance-budget.spec.ts。
   - steps：冷启动至少 30、热启动至少 100；测可交互 P95≤10s、输入 P95≤100ms、1920×1080 基础≥60FPS/诊断≥30FPS、阻塞≤200ms/进度≤2s、manifest P95≤500ms；软限提示、硬限拒绝。
   - command：npm run test:integration；npm run release:verify。
   - tests：F2S-TST-E2E-PERF-001、F2S-TST-E2E-PERF-002、F2S-TST-E2E-PERF-003、F2S-TST-E2E-PERF-004、F2S-TST-E2E-PERF-005、F2S-TST-E2E-PERF-007、F2S-TST-074、F2S-TST-086。
   - evidence：evidence/M09/F2S-DEV-M09-005/performance-summary.json、raw-traces/**。
   - dependsOn：[F2S-DEV-M01-003, F2S-DEV-M04-006, F2S-DEV-M07-003]。
   - parallelSafety：性能测试机独占；锁 repo:tools/benchmark、repo:tests/performance/performance-budget.spec.ts。
...
##### F2S-WU-M09-005-02 — B08渲染与8GB矩阵
   ...
   - reads：renderer、WebView2 exact version、DPI 100/125/150/200%、F2S-DEV-M09-001 Worker probe、8GB GPU。
   - writes：tests/performance/renderer-resilience.spec.ts、fixtures/performance/scenarios.json。
   - steps：每 DPI 测坐标/清晰度/延迟/FPS；注入 WebGL context loss 并重建，最后 committed revision 不丢；8GB 档预检 model/input/VRAM/RAM/temp，资源不足阻断或等待显式 tile/offload/CPU/较小模型选择并记录质量影响；禁止静默降质。
   - command：npm run test:integration；npm run build:ai-pack；npm run release:verify。
   - tests：F2S-TST-E2E-PERF-002、F2S-TST-E2E-PERF-003、F2S-TST-E2E-PERF-006、F2S-TST-086、F2S-TST-087、F2S-R3A-B08-P2-001。
   - evidence：evidence/M09/F2S-DEV-M09-005/evidence.json、b08-renderer-matrix.json、gpu8gb-preflight.json；缺 GPU/pack 格为 NOT_RUN/EXTERNAL。
   - dependsOn：[F2S-DEV-M07-003, F2S-DEV-M09-001]。
   - parallelSafety：GPU/GUI 测试机独占；锁 repo:tests/performance/renderer-resilience.spec.ts、repo:fixtures/performance/scenarios.json。
   - rollback：禁用未通过的 Worker/高级预览路径；Core 手工链与源资产保持。
   - estimate：1.5 人日。
7. **正向验收**：七项指标分别给出样本、P95/FPS/峰值与环境；B08 四维矩阵有逐格结果，context loss 后恢复且不丢 commit。
8. **负向与故障验收**：缺样本/环境、聚合掩盖单项失败、静默降质、真实硬件未跑却标 PASS、调低阈值均失败。
9. **证据**：evidence/M09/F2S-DEV-M09-005/evidence.json（逻辑 ID F2S-EVD-M09-005） 是 B08 关闭出口；原始 trace 与摘要 hash 绑定，外部硬件缺失明确 NOT_RUN/EXTERNAL。
...
##### F2S-WU-M09-006-01 — 受控故障注入Harness
   ...
   - evidence：evidence/M09/F2S-DEV-M09-006/fault-harness.json。
   - dependsOn：[F2S-DEV-M02-006, F2S-DEV-M02-007, F2S-DEV-M08-005]。
   - parallelSafety：NTFS 测试根与测试 child 独占；锁 repo:tools/fault-injection、repo:fixtures/fault。
...
##### F2S-WU-M09-006-02 — B11 NTFS原子矩阵
   ...
   - evidence：evidence/M09/F2S-DEV-M09-006/b11-ntfs-matrix.json、ntfs-fault.log。
   - dependsOn：[F2S-WU-M09-006-01]。
   - parallelSafety：单卷串行；锁 repo:tests/fault/ntfs-atomicity.spec.ts。
...
##### F2S-WU-M09-006-03 — 重启恢复与幂等重试
   ...
   - evidence：evidence/M09/F2S-DEV-M09-006/evidence.json、recovery-choice-report.json。
   - dependsOn：[F2S-WU-M09-006-02]。
   - parallelSafety：锁 repo:tests/fault/recovery-ui.spec.ts；每场景独立 projectId。
...
##### F2S-WU-M09-007-01 — 结构化本地日志
   ...
   - evidence：evidence/M09/F2S-DEV-M09-007/logger-tests.log、event-schema-report.json。
   - dependsOn：[F2S-DEV-M01-005, F2S-DEV-M02-005]。
   - parallelSafety：锁 repo:crates/adapters/diagnostics/src/lib.rs。
...
##### F2S-WU-M09-007-02 — 诊断包脱敏
   ...
   - evidence：evidence/M09/F2S-DEV-M09-007/evidence.json、redaction-canary-report.json。
   - dependsOn：[F2S-WU-M09-007-01, F2S-DEV-M09-001, F2S-DEV-M09-002]。
   - parallelSafety：锁 repo:crates/adapters/diagnostics/src/redaction.rs、repo:tests/privacy；只用 synthetic canary。
