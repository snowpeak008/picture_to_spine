...
revision: 1.0
status: draft
canonical_for: [F2S-DEVPLAN-CARDS-M10, F2S-WU-M10]
depends_on: [F2S-DOC-DEVPLAN-M08-001, F2S-DOC-DEVPLAN-M09-001]
review_score_ref: F2S-SCORE-DEVPLAN-M10-001-D0
...
4. **输出**：apps/desktop/src-tauri/tauri.conf.json、installer/nsis/core.nsi、installer/nsis/worker-pack.nsi、installer/nsis/model-pack.nsi、tests/packaging/package-layout.spec.ts；执行期输出 dist/windows/core/**、worker-pack/**、model-pack/**。
...
##### F2S-WU-M10-001-01 — Core安装布局
   ...
   - writes：apps/desktop/src-tauri/tauri.conf.json、installer/nsis/core.nsi。
   - steps：冻结 per-user install root；显式列 launcher/main/resources/licenses；检查系统 WebView2 并给稳定诊断；定义 install/repair/uninstall；排除用户项目和外部设置。
   - command：npm run build:core；npm run release:verify。
   - tests：普通用户安装、修复、卸载；WebView2 缺失；项目保留；包内开发工具/秘密扫描。
   - evidence：evidence/M10/F2S-DEV-M10-001/core-package-layout.json、release-verify.log。
   - dependsOn：[F2S-DEV-M09-004]。
   - parallelSafety：锁 repo:apps/desktop/src-tauri/tauri.conf.json、repo:installer/nsis/core.nsi、repo:dist/windows/core。
...
##### F2S-WU-M10-001-02 — 可选Pack分离
   ...
   - command：npm run build:ai-pack；npm test；npm run release:verify。
   - tests：F2S-TST-E2E-LIC-002、F2S-TST-076；注入禁止文件、错 pack hash、Worker sandbox 不合格。
   - evidence：evidence/M10/F2S-DEV-M10-001/evidence.json、pack-separation.json、forbidden-component-scan.json。
   - dependsOn：[F2S-DEV-M09-001, F2S-WU-M10-001-01]。
   - parallelSafety：锁 repo:installer/nsis/worker-pack.nsi、repo:installer/nsis/model-pack.nsi、repo:tests/packaging/package-layout.spec.ts。
...
##### F2S-WU-M10-002-01 — 原生定位与安全启动
   ...
   - command：npm run build:core；npm test。
   - tests：从任意 cwd、中文/空格/长路径双击；shell 元字符；main 缺失/替换；多实例。
   - evidence：evidence/M10/F2S-DEV-M10-002/native-launch-tests.json、build-core.log。
   - dependsOn：[F2S-DEV-M10-001]。
   - parallelSafety：锁 repo:apps/launcher-native/src/main.rs；不得并行覆盖根 FlashToSpineLauncher.exe。
...
##### F2S-WU-M10-002-02 — 身份与健康握手
   ...
   - command：npm run build:core；npm test；npm run test:integration。
   - tests：错签名/hash、重放 nonce、劫持 pipe、迟到握手、主程序崩溃、诊断无 shell。
   - evidence：evidence/M10/F2S-DEV-M10-002/health-handshake.json、identity-negative-tests.json。
   - dependsOn：[F2S-WU-M10-002-01, F2S-DEV-M09-003]。
   - parallelSafety：锁 repo:apps/launcher-native/src/verify.rs、repo:apps/launcher-native/src/health.rs；每测试随机 pipe。
...
##### F2S-WU-M10-002-03 — 根入口物化与签名状态
   ...
   - reads：F2S-WU-M10-002-01 与 F2S-WU-M10-002-02 binary、F2S-DEV-M10-006 签名输入合同与 publisher policy。
   ...
   - evidence：evidence/M10/F2S-DEV-M10-002/evidence.json、root-entry-smoke.json、signature-status.json；缺证书为 NOT_RUN/EXTERNAL。
   - dependsOn：[F2S-WU-M10-002-02]。
   - parallelSafety：锁 repo:tools/package-root-entry.ps1、repo:FlashToSpineLauncher.exe、repo:tests/launcher。
...
##### F2S-WU-M10-003-01 — 更新信任与反回滚
   ...
   - evidence：evidence/M10/F2S-DEV-M10-003/update-security.json、integration.log。
   - dependsOn：[F2S-DEV-M10-001, F2S-DEV-M10-006]。
   - parallelSafety：锁 repo:crates/adapters/update；每安装 identity 单更新 lease。
...
##### F2S-WU-M10-003-02 — 文件关联与升级保留
   ...
   - command：npm test；npm run test:integration；npm run release:verify。
   - tests：shell 元字符路径、关联劫持、错 ProgID、损坏/越界/内嵌资产链接、双击 .f2slink、升级/卸载项目保留。
   - evidence：evidence/M10/F2S-DEV-M10-003/evidence.json、file-association-matrix.json。
   - dependsOn：[F2S-DEV-M10-002, F2S-WU-M10-003-01]。
   - parallelSafety：锁 repo:installer/nsis/file-association.nsh、repo:tests/update/update-security.spec.ts；注册表测试用隔离用户/VM。
...
4. **输出**：apps/desktop-ui/src/platform/windows-a11y.ts、apps/desktop-ui/src/i18n/zh-CN.json、tests/ui/windows-accessibility.spec.ts、tests/ui/windows-path-dpi.spec.ts。
...
##### F2S-WU-M10-004-01 — 键盘焦点与简中
   ...
   - writes：apps/desktop-ui/src/platform/windows-a11y.ts、apps/desktop-ui/src/i18n/zh-CN.json、tests/ui/windows-accessibility.spec.ts。
   - steps：补语义名称/role/state；定义 Tab/方向键/快捷键与 modal focus trap/restore；错误/审批/命中状态加文本/图形；检查硬编码文本与资源缺键。
   - command：npm run typecheck；npm test；npm run test:integration。
   - tests：F2S-TST-E2E-A11Y-001、F2S-TST-E2E-A11Y-002、F2S-TST-E2E-A11Y-003、F2S-TST-E2E-I18N-001、F2S-TST-088。
   - evidence：evidence/M10/F2S-DEV-M10-004/a11y-i18n-report.json、keyboard-flows.json。
   - dependsOn：[F2S-DEV-M07-006]。
   - parallelSafety：锁 repo:apps/desktop-ui/src/platform/windows-a11y.ts、repo:apps/desktop-ui/src/i18n/zh-CN.json、repo:tests/ui/windows-accessibility.spec.ts。
...
##### F2S-WU-M10-004-02 — 路径DPI与对比矩阵
   ...
   - evidence：evidence/M10/F2S-DEV-M10-004/evidence.json、path-dpi-matrix.json、contrast-report.json。
   - dependsOn：[F2S-DEV-M09-005, F2S-WU-M10-004-01]。
   - parallelSafety：GUI 测试机独占；锁 repo:tests/ui/windows-path-dpi.spec.ts。
...
##### F2S-WU-M10-005-01 — Clean VM矩阵Harness
   ...
   - command：npm run bootstrap:check；npm run test:integration。
   - tests：Harness 对错 VM/hash、非 clean、管理员账户、缺 WebView2 的拒绝。
   - evidence：evidence/M10/F2S-DEV-M10-005/harness-attestation.json。
   - dependsOn：[F2S-DEV-M10-001, F2S-DEV-M10-002, F2S-DEV-M10-003, F2S-DEV-M10-004]。
   - parallelSafety：每 VM 独占；锁 repo:tools/windows-matrix、repo:tests/windows-matrix/scenarios.json。
...
##### F2S-WU-M10-005-02 — Windows11 P0矩阵
   ...
   - evidence：evidence/M10/F2S-DEV-M10-005/win11-p0.json、win11-artifacts-index.json。
   - dependsOn：[F2S-WU-M10-005-01, F2S-DEV-M09-005, F2S-DEV-M09-006]。
   - parallelSafety：Win11 VM 独占；锁 repo:tests/windows-matrix/assertions.spec.ts。
...
##### F2S-WU-M10-005-03 — Windows10 P1矩阵
   ...
   - evidence：evidence/M10/F2S-DEV-M10-005/evidence.json、win10-p1.json、known-limitations.json。
   - dependsOn：[F2S-WU-M10-005-01]。
   - parallelSafety：Win10 VM 独占；证据目录由本 WU 独占。
...
##### F2S-WU-M10-006-01 — 发布Manifest与签名合同
   ...
   - evidence：evidence/M10/F2S-DEV-M10-006/manifest-contract.json、synthetic-signature-tests.json。
   - dependsOn：[F2S-DEV-M09-004, F2S-DEV-M10-001, F2S-WU-M10-002-02]。
   - parallelSafety：锁 repo:tools/signing/build-application-manifest.ps1、repo:tests/signing/signature-policy.spec.ts；dist 只读。
...
##### F2S-WU-M10-006-02 — 真实证书条件签名与验签
   ...
   - evidence：evidence/M10/F2S-DEV-M10-006/evidence.json、signature-report.json；证书/正式 enrollment 缺失为 NOT_RUN/EXTERNAL。
   - dependsOn：[F2S-WU-M10-006-01, F2S-WU-M10-002-03]。
   - parallelSafety：签名 staging 与证书 session 独占；锁 repo:tools/signing/verify-signatures.ps1。
