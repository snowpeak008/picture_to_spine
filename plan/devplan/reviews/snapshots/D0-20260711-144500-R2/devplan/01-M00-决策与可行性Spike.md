...
- steps: 1) 建最小 bone/slot/skin；2) 固定 Spine skeleton version 4.2.43；3) 领域与 Rig IR 使用 i64 tick+rational timeBase（默认 1/30000 秒），仅在 Spine42 Adapter 边界把 tick 转为秒；4) 只引用透明 PNG；5) 规范排序与 float；6) 写 golden hash。
- command: npm run test:spine
